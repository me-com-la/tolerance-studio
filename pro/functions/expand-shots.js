...
async function expandShots(deps, params) {
  ...
  // Percentage-of-frame placement (Owner call, 2026-07-09 — replaces the old
  // inches-based dimensions hint, which only ever fed a vague sentence into
  // this prompt and never actually controlled render scale). Copied onto
  // every shot's item.placement; generate.js's compositeProductOntoBackground
  // reads it directly as a fraction of the background's height, falling back
  // to its own randomized 0.6-0.9 range when the Owner leaves it unset.
  const placement = project.tags?.placement || null;
  ...
  const system =
    ...
    'stated first; (2) the style lock — must/should-weighted creative constants, stated as ' +
