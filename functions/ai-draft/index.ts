...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (kind === 'tags') {
      ...
      // 2026-07-16: the "+" row was drafting scenes almost identical to what
      // was already on screen because it sent zero context about the existing
      // list. Same avoid-list pattern the copy path already uses — client
      // ships the current scene phrases in `avoid`, the prompt tells the
      // model to take a genuinely different setting/situation.
      const avoidScenes = Array.isArray(avoid) ? avoid.filter((s)=>typeof s === 'string' && s.trim()).slice(0, 12) : [];
      const system = 'You draft the Scenes list for an AI product-photography batch: a numbered list of short scene ' + 'phrases (5-10 words each), one per line, count = the batch size. Each scene describes only the ' + "setting/situation — the product and creative-constant details from the brief tags get combined " + "with each scene later when the shots are expanded, so don't repeat them here. Written for an " + 'Owner who edits it by hand for variety — quick to scan, quick to change. When an ' + '"avoid" list is provided, every scene you draft must be a genuinely different setting, ' + 'situation, time of day, or spatial context from those — not a rewording. Reach for a ' + 'different room, exterior, activity, angle, or lighting rather than another variant of the same idea.';
      const user = `${briefLine}\n\nBrief tags (weighted; must > should > flavor):\n${brief}\n\nPrevious scene lists from this client (keep the brand ` + `feel consistent, but this campaign needs its own variety):\n${prevTxt}\n\n` + (direction ? `Owner direction: ${direction}\n\n` : '') + (avoidScenes.length ? `Do NOT repeat these scenes already in this batch — take genuinely different settings/situations, not synonyms of them:\n${avoidScenes.map((s)=>`- ${s}`).join('\n')}\n\n` : '') + 'Write the Scenes list only, as a numbered list of short phrases, count = ' + batchSize + '. No heading, no other text, no product/style detail restated.';
      const text = await callFal(falKey, system, user);
