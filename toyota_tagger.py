#!/usr/bin/env python3
"""
Tag Toyota images using Apple Vision — detects people and faces.
Adds tags to manifest.json: 'people', 'face'
Safe to re-run: skips images already tagged (has 'tagged' key).

Usage:
  python3 toyota_tagger.py
  python3 toyota_tagger.py --retag   # re-run even if already tagged
"""
...
def detect_image(img_path: Path) -> list[str]:
    """Run Vision on img_path, return list of detected tags."""
    ...
    # Detect people (full body rectangles)
    ...
    if face_req.results() and len(face_req.results()) > 0:
        tags.add("face")
        tags.add("people")  # face implies people
...
def main():
    ...
    for uid, entry in manifest.items():
        ...
        # save every 50 images in case of interruption
        if total % 50 == 0:
